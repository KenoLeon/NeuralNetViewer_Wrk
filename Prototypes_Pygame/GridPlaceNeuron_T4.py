...
class Neuron():
    ...
    def draw(self):
        if self.place:
            self._rect = pygame.draw.circle(DISPLAYSURF, GREY, (self.x,self.y), NEURONSIZE, 1)
        else:
...
def checkNeurons():
     ...
     for neuron in NEURON_LIST:
         if neuron._rect.collidepoint(pygame.mouse.get_pos()) and not neuron.place :
             hovering = True
             pygame.draw.circle(DISPLAYSURF, GREY, (neuron.x,neuron.y), NEURONSIZE)
             if pygame.mouse.get_pressed()[0]:
                pygame.draw.circle(DISPLAYSURF, DARKGREY, (neuron.x,neuron.y), NEURONSIZE)
